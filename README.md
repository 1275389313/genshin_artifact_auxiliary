...
Windows 下装好上述依赖后，在仓库根目录执行：

```
pyinstaller app.spec
```

产物在 `dist/keqing-1.0.0/`，入口为 `keqing.exe`。**运行仍须管理员权限**（监听游戏内鼠标与快捷键）。spec 保留 `console=True`，方便对照下方「问题反馈」提供命令行输出。请保持 `datas=[('src', 'src')]`。

打包后默认配置、图标等从程序自带的 `src/` 读取（PyInstaller 的 `sys._MEIPASS`），**不依赖当前工作目录**。管理员运行时 Windows 常把 cwd 设为 `C:\Windows\System32`，相对路径 `src/character.json` 会找不到并闪退。OCR 调试截图写到 `%TEMP%/keqing_ocr/`（`grab.png` / `out.png`），不写进只读的打包 `src/`。个人配置仍只写在 `文档/keqing/`。

## 使用教程

1. 打开游戏，手动在设置-图像-显示模式中调整为窗口或无边框。现已支持 **16:9 / 16:10** 窗口与全屏无边框，**含 4K（3840×2160）**，不必再强制改成 1920×1080。**原神不能最小化**（任务栏图标即可，不要点最小化）；每次 F8 /「扫描五件」前会按当前窗口重算坐标，**改分辨率后无需死磕重启工具**。建议无边框或稳定窗口模式；系统缩放 150%/200% 已按 DPI 处理。须**管理员**运行。
...
主窗口角色下拉按上线时间排序（新→旧，占位项在最前），与 json 键顺序无关；见 [文档/有效词条.md](文档/有效词条.md)。

## 已知问题

### 分辨率适配

已适配游戏窗口 16：10，16：9（含 4K 无边框/窗口），坐标按游戏窗口比例缩放，启动时会设 Per-Monitor DPI 感知，避免 150%/200% 缩放把 4K 窗口比例算错。扫描前会按当前窗口重算点击/OCR/贴图坐标，改分辨率或窗口大小后不必重启工具。**原神不能最小化**；最小化时启动不会再卡死，扫描会提示先还原窗口。若有分辨率适配问题，可查看 `%TEMP%/keqing_ocr/`（Windows 临时目录）中的 `grab.png`、`out.png` 识别截图是否准确，并提供一些不同分辨率的截图做坐标定位和测试了。

**4K 可用前提：** 游戏窗口本身是 16:9 或 16:10；原神不要最小化；管理员运行；DPI 已由程序处理。仍建议无边框或稳定窗口模式。
