...
4. 在主窗口中选择角色（不选的话，默认评估双爆+攻击词条，即「常规主C-攻暴」）。
5. 打开背包-圣遗物或角色-圣遗物装配（需在主程序中对应选择背包或角色）。**贴图主数字是有效词条**（一位小数）。背包：右键识别单件。角色装配页：按 **F8**（可改）或点「扫描五件」，依次点击花→羽→沙→杯→冠并识别，合计显示为 `有效词条 28.4`。**F8 时原神需在最前**；工具会尝试自动置顶。
...
**Windows 冒烟（无法在 CI 开原神）：** 4K 无边框 16:9 启动后日志应出现 `物理桌面(3840, 2160)`、`ratio≈1.78 → 16:9`，而不是「请将游戏显示模式调至1920*1080」。角色装配页 F8 五件扫描，贴图应落在花/羽/沙/杯/冠；若系统为 200% 缩放，再确认一次点击与贴图仍对齐。
