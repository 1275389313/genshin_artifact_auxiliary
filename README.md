...
产物在 `dist/keqing-1.0.0/`，入口为 `keqing.exe`。**运行仍须管理员权限**（监听游戏内鼠标与快捷键）。spec 保留 `console=True`，方便对照下方「问题反馈」提供命令行输出。
...
已适配游戏窗口 16：10，16：9（含 4K 无边框/窗口），坐标按游戏窗口比例缩放，启动时会设 Per-Monitor DPI 感知，避免 150%/200% 缩放把 4K 窗口比例算错。扫描前会按当前窗口重算点击/OCR/贴图坐标，改分辨率或窗口大小后不必重启工具。**原神不能最小化**；最小化时启动不会再卡死，扫描会提示先还原窗口。若有分辨率适配问题，可查看程序目录中的`src/grab.png``src/out.png`识别截图是否准确，并提供一些不同分辨率的截图做坐标定位和测试了。
